...
class Link{
	...
	static async list(){
		let sql = `select * from link order by id asc`;
	...
	static async showEdit({id}){
		let sql = `select * from link where id= "${id}"`;
		return await db.q(sql);
	}

	static async edit({id,linkName,linkUrl}){
		let sql = `update link set link_name="${linkName}",link_url="${linkUrl}"
			where id="${id}"`;
		await db.q(sql);
	}
}
